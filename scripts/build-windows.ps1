...
param(
    ...
    # Build and run the ported upstream suites through ctest after installing.
    # Implies -IncludePortedTests; tests are skipped by default.
    [switch] $RunPortedTests
)
...
if ($RunPortedTests) { $IncludePortedTests = $true }
...
$scriptLibraryName = if ($effectiveConfiguration -eq 'Debug') {
    'Qt6Scriptd.dll'
} else {
    'Qt6Script.dll'
}
$scriptLibrary = Join-Path $QtRoot "bin\$scriptLibraryName"
